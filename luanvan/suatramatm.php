...
<body>
	...
	$a=$_POST['Sua'];
		...
		$query ="select * from tramatm where MaTramATM = '$a'";
		$result = mysqli_query($conn, $query);
		$d=mysqli_fetch_array($result);
		
			//kiem tra các trường nhập có để trống
	if( !isset($_POST['TenTramATM']) || !isset($_POST['DiaChi']) || !isset($_POST['Rating']) || !isset($_POST['Lng']) ||!isset($_POST['Lat'])){
		echo "Bạn hãy sữa thông tin trạm ATM";
	}
	else{
		...
		$query = "SELECT * FROM nganhang WHERE TenNH='".$tennganhang."'";
		$result = mysqli_query($conn, $query);
		...
		//khi nhan nut sửa
		$sql="UPDATE tramatm SET TenTramATM='$ten',
			DiaChi='$diachi',
			Rating='$rating',
			Lat='$lat',
			Lng='$lng',
		...
		header("location:tramatm.php");
		exit();
	}
	?>
	<form action= ""  method="post" align="center">
				Tên Trạm ATM:</br>  <input type="text" name = "TenTramATM" value="<?php echo $d['TenTramATM'];?>"></br>
							...
							<?php
							 $rows = array();
								 while ($row = $row = mysqli_fetch_assoc($result)) {
									 if($row["MaNH"] == $d["MaNH"]){
							?>
									  <option  selected="selected"><?php echo $row["TenNH"];?></option>
							  <?php  
							  } else{
								  ?>
								  <option ><?php echo $row["TenNH"];?></option>
							   <?php  }
								 }
										mysqli_close($conn);
							 ?>
